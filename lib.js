...
function nx_export_data(request, sender, sendResponse) {
    ...
    tHTML = tHTML + '<link href="style.css">';
    ...
    tHTML = tHTML + b.join("") + '</div></div></div><script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js"></script></body></html>';
    win.document.open();
...
function build_result_html(title, )
{
    title = c.aAuth + (c.aAuth ? "-" : "") + c.aName;

    var tHTML = '<html><body class="index"><script>document.title="' + title + '";</script>';
